...
`Security` tab of this repository and submit a report there. The advisory draft
is visible only to the maintainers and the reporter.

Fallback if GitHub PVR is unavailable: email `security@kadenz.live` with the
subject `[security] kadenz-scanner vulnerability report`.
